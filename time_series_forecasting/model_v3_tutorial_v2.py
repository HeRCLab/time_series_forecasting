...
class TimeSeriesForcasting(pl.LightningModule):
    def __init__(
        ...
        self.input_pos_encoder = PositionalEncoding(channels, dropout)
        self.input_embedding = nn.Embedding(no_unique_vals_in_data, channels)

        self.target_pos_encoder = PositionalEncoding(channels, dropout)
        self.target_embedding = nn.Embedding(no_unique_vals_in_data, channels)
    ...
    def encode_src(self, src):
        ...
        #
        # # may try with in_sequence_len = 5000 as per v1 tutorial
        # pos_encoder = (
        #     torch.arange(0, in_sequence_len, device=src.device)
        #     # torch.arange(0, 5000, device=src.device)
        #     .unsqueeze(0)
        #     .repeat(batch_size, 1)
        # )
        ...
        src = self.input_embedding(src) #* math.sqrt(NO_CHANNELS)
        src = torch.squeeze(src, 3)
        print('@encode_src 2. src.size() = ', src.size())
        print('@encode_src 2. src = ', src)

        src = self.input_pos_encoder(src).permute(1, 0, 2)
        print('@encode_src 3. src.size() = ', src.size())
        print('@encode_src 3. src = ', src)

        src = self.encoder(src)
        print('@encode_src 4. src.size() = ', src.size())
        print('@encode_src 4. src = ', src)
    ...
    def decode_trg(self, trg, memory):
        ...
        # out_sequence_len = trg_start.size(0)
            # , batch_size = trg_start.size(0), trg_start.size(1)
        # print('@decode_trg out_sequence_len = ', out_sequence_len)
        # print('@decode_trg batch_size = ', batch_size)


        # pos_decoder = (
        #     torch.arange(0, out_sequence_len, device=trg.device)
        #     # torch.arange(0, 5000, device=trg.device)
        #     .unsqueeze(0)
        #     .repeat(batch_size, 1)
        # )
        ...
        trg_mask = gen_trg_mask(self.no_unique_vals_in_data, trg.device) #ifty

        trg = self.target_embedding(trg) #* math.sqrt(NO_CHANNELS)
        trg = torch.squeeze(trg, 3)

        trg = self.target_pos_encoder(trg).permute(1, 0, 2)
