...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if IF_USE_SLIDING_WINDOW_DATASET:
            ...
            for index in range(0, len(self.source_data_list) - self.feature_len):
                # print('i = ', i)
                # print('index = ', index)
                src_segment = self.source_data_list[index:index + self.feature_len]
                src.append(src_segment)
                # print('tmplist = ', tmplist)
            ...
            for index in range(0, len(self.target_data_list) - self.feature_len):
                # print('i = ', i)
                # print('index = ', index)
                trg_in_segment = self.target_data_list[index:index + self.feature_len]
                trg_in.append(trg_in_segment)
                    # print('tmplist = ', tmplist)
            ...
            for indx, stepindx in enumerate(range(0, len(self.source_data_list_name), self.feature_len)):
                ...
                src_segment = self.source_data_list[stepindx:stepindx + self.feature_len]
                src.append(src_segment)
                # src DONE

                trg_in_segment = self.target_data_list[stepindx:stepindx + self.feature_len]
                trg_in.append(trg_in_segment)
                # trg_in DONE

                trg_out.append(self.target_data_list[stepindx + self.feature_len]) # DONE
...
def train(
    ...
    model = TimeSeriesForcasting(
        ...
        no_unique_vals_in_data= 30657, #40,#30657, #no_unique_vals_in_data,
        lr=1e-5,
        dropout=0.1,
    )
