...
MAX_LEN_PositionalEncoding = 11
# ifty pl.LightningModule
# from v1 tutorial
# class PositionalEncoding(nn.Module):
class PositionalEncoding(pl.LightningModule):

    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = MAX_LEN_PositionalEncoding):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)

        self.position = torch.arange(max_len, device=torch.device('cuda')).unsqueeze(0).repeat(128, 1)
    ...
    def forward(self):
        ...
        # x = x + self.pe[:x.size(0)]
        return self.position #self.dropout(x)
...
class TimeSeriesForcasting(pl.LightningModule):
    def __init__(
        ...
        # self.input_pos_embedding = PositionalEncoding(channels, dropout)
        # self.target_pos_embedding = PositionalEncoding(channels, dropout)

        self.input_pos_embedding = PositionalEncoding(n_encoder_inputs, dropout)
        self.target_pos_embedding = PositionalEncoding(n_decoder_inputs, dropout)

        self.embedding = torch.nn.Embedding(no_unique_vals_in_data, embedding_dim=channels)
    ...
    def encode_src(self, src):
        ...
        print('@encode_src 1. src_start.size() = ', src_start.size())
        ...
        # src = self.embedding(src) * math.sqrt(self.channels)
        # src = self.input_pos_embedding(src).permute(1, 0, 2)

        src = self.input_pos_embedding()
        src = self.embedding(src).permute(1, 0, 2)


        print('@encode_src 1. src_start.size() = ', src_start.size())
        print('@encode_src 1. src.size() = ', src.size())
    ...
    def decode_trg(self, trg, memory):
        ...
        # src = self.input_pos_embedding(src)
        #
        # # src = src_start + pos_encoder
        # src = src_start + src
        ...
        # trg = self.embedding(trg) * math.sqrt(self.channels)
        # trg = self.target_pos_embedding(trg).permute(1, 0, 2)

        trg = self.target_pos_embedding()
        trg = self.embedding(trg).permute(1, 0, 2)
        ...
        trg_mask = gen_trg_mask(MAX_LEN_PositionalEncoding, trg.device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        print('@validation_step src.size() = ', src.size())
        print('@validation_step trg_in.size() = ', trg_in.size())
        print('@validation_step trg_out.size() = ', trg_out.size())

        y_hat = self((src, trg_in))
        print('@validation_step y_hat.size() = ', y_hat.size())

        y_hat = y_hat.view(-1)
        print('@validation_step y_hat.size() = ', y_hat.size())

        y = trg_out.view(-1)
        print('@validation_step y.size() = ', y.size())
